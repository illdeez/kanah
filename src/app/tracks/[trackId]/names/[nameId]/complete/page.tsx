...
export default function CompleteNamePage() {
  ...
  function handleSaveLine() {
    ...
    saveReflection(trackId, storyIdForStorage, selectedLine);
    setStep("pledge");
  }
    ...
    <main className="flex flex-col min-h-screen px-6 pt-14 pb-8">
      ...
      >
        <span className="text-[11px] font-semibold text-kanah-completed tracking-widest uppercase block mb-8">
          ✓ أتممت {name.name} · {toArabicNumeral(name.number)}
        </span>
          ...
          {step === "line" ? (
            ...
            >
              <p className="text-[12px] text-kanah-muted leading-[1.8]">
                سيُحفَظ اختيارك تحت {name.name} · {name.title}
              </p>
              {name.selectedLines.length > 0 ? (
                <motion.div
                ...
                <div className="rounded-2xl border border-kanah-border bg-kanah-card p-6 text-center text-kanah-muted text-[14px]">
                  لا توجد جمل مختارة لهذا الاسم بعد
                </div>
              )}
            </motion.div>
          ) : (
        ...
        <AnimatePresence mode="wait">
          {step === "line" ? (
            <motion.button
              ...
              disabled={!selectedLine && name.selectedLines.length > 0}
              whileTap={selectedLine || name.selectedLines.length === 0 ? { scale: 0.97 } : {}}
            ...
            >
              {name.selectedLines.length > 0 ? "احفظها في أثري" : "التالي"}
            </motion.button>
          ) : (
            <motion.button
              key="save-pledge"
