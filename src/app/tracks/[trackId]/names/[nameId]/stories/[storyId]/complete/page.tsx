...
export default function CompleteNameStoryPage() {
  ...
  function handleSavePledge() {
    ...
    if (!pledgeText.trim()) {
      setSaved(true);
      setTimeout(() => router.replace(`/tracks/${trackId}/names/${nameId}/stories`), 1000);
      return;
    }
  ...
  return (
    <main className="flex flex-col min-h-screen px-6 pt-14 pb-8" dir="rtl">
      {/* Header */}
      <motion.div
      ...
      >
        <span className="text-[11px] font-semibold text-kanah-completed tracking-widest uppercase block mb-8">
          ✓ أتممت {name.name} · اليوم {toArabicNumeral(story.storyNumber)}
        </span>
        ...
        <AnimatePresence mode="wait">
          {step === "line" ? (
            ...
            >
              ...
              <p className="text-[14px] text-kanah-muted leading-[1.9]">
                خذ من الاسم خطوة صغيرة تحوّل المعنى إلى سلوك.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {step === "line" ? (
            ...
            >
              <p className="text-[12px] text-kanah-muted leading-[1.8]">
                سيُحفَظ اختيارك تحت {name.name} · {story.title}
              </p>
              {story.selectedLines.length > 0 ? (
                <motion.div
                ...
                <div className="rounded-2xl border border-kanah-border bg-kanah-card p-6 text-center text-kanah-muted text-[14px]">
                  لا توجد جمل مختارة لهذه القصة بعد
                </div>
              )}
            </motion.div>
          ) : (
        ...
        <AnimatePresence mode="wait">
          {step === "line" ? (
            <motion.button
              ...
              disabled={!selectedLine && story.selectedLines.length > 0}
              whileTap={selectedLine || story.selectedLines.length === 0 ? { scale: 0.97 } : {}}
            ...
            >
              {story.selectedLines.length > 0 ? "احفظها في أثري" : "التالي"}
            </motion.button>
          ) : (
            <motion.button
              key="save-pledge"
